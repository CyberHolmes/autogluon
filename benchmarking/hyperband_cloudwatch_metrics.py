...
parser.add_argument('--out', default='output/resource_util/ag1_2', help='output file')
...
jctlog = pd.read_csv("./output/jct/autog1_hyperband_2/hyperband_benchmark.csv")
#print(jctlog)
...
EC2_instances = ['i-03d66937849994c6a']
    ...
    for j in range(jctlog.shape[0]):
        ...
        for datapoint in Datapoints1:
            results_cpu = results_cpu.append({
                ...
                'bracket': row['bracket'].iloc[0],
                'reduction_factor': row['reduction_factor'].iloc[0],
...
results_cpu.to_csv(args.out+'/'+'hyperband_cpu_util.csv')
results_gpu.to_csv(args.out+'/'+'hyperband_gpu_util.csv')